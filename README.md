...
This project is a menu for Chabad house, with order flow, charging and kitchen prints.

## Development

For local development, create a `.env.local` file in the root folder. This file will NOT be overwritten by Terraform and is ignored by git. Example:
...
PRINTER_IP=<ip-of-your-printer>

### Backup ###
BACKUP_TIME=0 0 * * *
BACKUP_TIMEZONE=<timezone | Asia/Jerusalem>
NODE_ENV=dev
```

## Production

For production, the `.env` file is generated by Terraform and should NOT be edited manually. It contains only the variables needed for production, and uses S3-specific credentials for S3 access, while the EC2 instance role is used for CloudWatch logging.

## Server

The server uses `NodeJS` and the `DB` is `sqlite3`. To run the backend for local development, use:

```bash
npm run start:dev
```

If you have errors about creating tables, try:
...
Now, you should have a backend running with your admin user and some mocked items.
...
To deploy the app server on AWS this project includes the 'tf' directory.
...
* A reverse proxy for serving the app with auto-renewing SSL certificates for HTTPS (optional, requires a privately owned domain) - read more in the next sections

Terraform will generate a `.env` file in the project root dir with the generated AWS keys for production.
